...
if ($vswhere.instances.length -eq 0) {
    Write-Error "No VS installation detected that has all the required workloads installed ($workloadsRequired). Use $PSScriptRoot\Install-VS.ps1 to acquire VS with the required components."
    exit 2
}
