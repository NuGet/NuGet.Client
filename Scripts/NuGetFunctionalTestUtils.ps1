...
function WriteToTeamCity
{
    ...
    {
        Write-Host -ForegroundColor Red 'WARNING: PARSING ISSUES. CANNOT WRITE TO TEAM CITY!'
        return $false
    }
    else
    {
        ...
        if ($status -eq "Failed")
        {
            ...
            {
               Write-Host -ForegroundColor Red 'WARNING: PARSING ISSUES. CANNOT WRITE TEST FAILURE TO TEAM CITY!'
            }
            else
            {
