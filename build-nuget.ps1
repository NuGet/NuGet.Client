...
{
    pushd "$GitRoot\PackageManagement"
    ...
    & "$GitRoot\PackageManagement\pack.ps1" @args
    $result = $lastexitcode
	
	# ILMerge nuget.exe
	cd "$GitRoot\PackageManagement\src\NuGet.CommandLine\bin\$Configuration"		
	if (Test-Path Merged)
	{	 
	    rmdir Merged\nuget.exe
	}
...
{
    pushd "$GitRoot\VisualStudioExtension"
    ...
    & msbuild VisualStudioExtension.sln /p:Configuration=$Configuration /p:VisualStudioVersion="14.0" /p:DeployExtension=false /p:PublicRelease=$Release
    if ($LASTEXITCODE -ne 0)
    {
        popd
        throw "Build failed"
