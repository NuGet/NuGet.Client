...
1. Create a [pull request](https://github.com/NuGet/NuGet.Client/pulls).
    * Keep the pull request template, and link to an issue [NuGet/Home](https://github.com/NuGet/Home/issues). Create a new issue if you cannot find an existing one. Use a meaningful PR title, not the autogenerated title based on the branch name.
