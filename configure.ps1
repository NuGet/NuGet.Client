...
Function New-BuildToolset {
    ...
    if ($ToolsetVersion -eq 15) {
        ...
        if ($InstallationPath) {
            $ToolsetObject = @{
                VisualStudioInstallDir = Join-Path $InstallationPath 'Common7\IDE'
            }
        }
    }

    $CommonToolsVar = "Env:VS${ToolsetVersion}0COMNTOOLS"
    if (-not $ToolsetObject -and (Test-Path $CommonToolsVar)) {
...
Invoke-BuildStep 'Validating VS15 toolset installation' {
    ...
    if ($vs15) {
        ...
        if (-not (Test-Path $Targets)) {
            ...
            $VSToolsPath = Join-Path $vs15.VisualStudioInstallDir '..\..\MSBuild\Microsoft\VisualStudio\v15.0'
            if (Test-Path $VSToolsPath) {
                $ConfigureObject.Add('EnvVars', @{ VSToolsPath = [System.IO.Path]::GetFullPath($VSToolsPath) })
            }
        }
    }
