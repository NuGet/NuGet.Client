...
namespace NuGet.PackageManagement.VisualStudio
{
    ...
    public sealed class LegacyPackageReferenceProject : BuildIntegratedNuGetProject
    {
        ...
        public async Task<(IReadOnlyList<PackageReference>, IReadOnlyList<PackageReference>)> GetAllPackagesAsync(CancellationToken token)
        {
            ...
            var packageSpec = await GetPackageSpecAsync(NullSettings.Instance);
            return await GetAllPackageReferencesAsync(packageSpec);
        }
        ...
        private async Task<IReadOnlyList<PackageReference>> GetPackageReferencesAsync(PackageSpec packageSpec)
        {
            var packages = await GetAllPackageReferencesAsync(packageSpec);
        ...
        private IReadOnlyList<PackageReference> GetTransitivePackageReferences(NuGetFramework targetFramework, Dictionary<string, ProjectInstalledPackage> installedPackages, Dictionary<string, ProjectInstalledPackage> transitivePackages, PackageSpec assetsPackageSpec, IList<LockFileTarget> targets)
        {
            ...
            else
            {
                return targets
                    ...
                    .SelectMany(library => GetPackageReferenceUtility.UpdateTransitiveDependencies(library, targetFramework, assetsPackageSpec?.TargetFrameworks.FirstOrDefault(), targets, installedPackages, transitivePackages))
