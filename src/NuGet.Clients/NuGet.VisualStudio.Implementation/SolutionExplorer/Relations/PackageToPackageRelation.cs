...
namespace NuGet.VisualStudio.SolutionExplorer
{
    ...
    internal sealed class PackageToPackageRelation : RelationBase<PackageReferenceItem, PackageReferenceItem>
    {
        ...
        protected override IEnumerable<PackageReferenceItem>? CreateContainedByItems(PackageReferenceItem child)
        {
            if (child.Target.TryGetDependents(child.Library.Name, out ImmutableArray<AssetsFileTargetLibrary> dependents))
            {
                return dependents
                    .Where(dep => dep.Type == AssetsFileLibraryType.Package)
                    .Select(library => new PackageReferenceItem(child.Target, library));
            }
