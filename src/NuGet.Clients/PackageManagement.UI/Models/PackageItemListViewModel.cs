...
namespace NuGet.PackageManagement.UI
{
    ...
    {        
        ...
        static Dictionary<Uri, BitmapImage> bitmapImageCache = new Dictionary<Uri, BitmapImage>();

        // this is called when users move away from "Browse" tab.
        public static void ClearBitmapImageCache()
        {
            bitmapImageCache = new Dictionary<Uri, BitmapImage>();
        }

        private static int iconLoadAttempts = 0;
        ...
        public BitmapImage IconBitmapImage
        {
            get
            {
                if (IconUrl != null)
                {
                    ...
                    {
                        if (bitmapImageCache.ContainsKey(IconUrl))
                        {
                            iconBitmapImage = bitmapImageCache[IconUrl];
                        ...
                        else
                        {
                            ...
                            if (iconFailures < 5 || ((double)iconFailures / iconLoadAttempts) < stopLoadingImageThreshold)
                            {
                                ...
                                bitmapImageCache[IconUrl] = iconBitmapImage;
                            ...
                            {
                                bitmapImageCache[IconUrl] = DefaultPackageIcon;
                            }
                        }
                    }
                }
        ...
        private void IconBitmapImage_DownloadOrDecodeFailed(object sender, System.Windows.Media.ExceptionEventArgs e)
        {
            ...
            if (bitmapImageCache[bitmapImage.UriSource] != DefaultPackageIcon)
            {
                bitmapImageCache[bitmapImage.UriSource] = DefaultPackageIcon;
                iconFailures++;
            }
        }
