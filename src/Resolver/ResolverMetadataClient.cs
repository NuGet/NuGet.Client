...
namespace NuGet.Resolver
{
    public static class ResolverMetadataClient
    {
        public static Task<RegistrationInfo> Load(HttpClient httpClient, string targetFramework, bool includePrerelease, IList<PackageEntry> currentInventory, PackageEntry packageToInstall)
        {
            ...
            application.Install(packageToInstall);

            return application.Load(httpClient);
        }
        ...
        public static async Task<RegistrationInfo> GetTree(HttpClient httpClient, RegistrationInfo registrationInfo, Func<IDictionary<NuGetVersion, HashSet<string>>, IDictionary<NuGetVersion, HashSet<string>>> filter = null)
        {
            ConcurrentDictionary<Uri, JObject> sessionCache = new ConcurrentDictionary<Uri, JObject>();
                    ...
                    if (range.Satisfies(packageVersion))
                    {
                        ...
                        if (dependencyGroupsArray != null)
                        {
                            foreach (JObject dependencyGroupObj in dependencyGroupsArray)
                            {
                                ...
                                dependencyGroupInfo.TargetFramework = (dependencyGroupObj["targetFramework"] != null) ? dependencyGroupObj["targetFramework"].ToString() : "any";
        ...
        static bool IsItemRangeRequired(VersionRange preFilterRange, NuGetVersion lower, NuGetVersion upper)
        {
