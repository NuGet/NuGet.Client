...
namespace NuGet.Commands
{
    ...
    public class ProjectFactory : MSBuildUser
    {
        ...
        private const string BuildFailure = "1";
        ...
        public PackageBuilder CreateBuilder(string basePath)
        {
            ...
            Logger.LogMinimal(
                string.Format(
                    ...
                    Strings.PackagingFilesFromOutputPath,
            ...
            catch (Exception ex)
            {
                Logger.LogWarning(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        Strings.UnableToExtractAssemblyMetadata,
            ...
            if (String.IsNullOrEmpty(builder.Description))
            {
                ...
                Logger.LogWarning(
                    string.Format(
                        CultureInfo.CurrentCulture,
                        Strings.Warning_UnspecifiedField,
                        "Description",
                        "Description"));
            }
        ...
        private void BuildProject()
        {
            if (Build)
            {
                if (TargetFramework != null)
                {
                    Logger.LogMinimal(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.BuildingProjectTargetingFramework,
                            _project.FullPath,
                            TargetFramework));
                }
            ...
            else
            {
                ...
                {
                    throw new PackagingException(String.Format(CultureInfo.CurrentCulture, Strings.UnableToFindBuildOutput, TargetPath));
                }
            }
        }
        ...
        private void BuildProjectWithMsbuild()
        {
            ...
            using (dynamic projectCollection = Activator.CreateInstance(projectCollectionType))
            {
                ...
                if (BuildFailure.Equals(result.OverallResult.ToString(), StringComparison.OrdinalIgnoreCase))
                {
                    // If the build fails, report the error
                    throw new PackagingException(Strings.FailedToBuildProject, Path.GetFileName(_project.FullPath));
                }
        ...
        private object CreateLoggers()
        {
            ...
            verbosityProperty.SetMethod.Invoke(consoleLogger, new object[] { Quiet }); ;
        ...
        private PackageDependency CreateDependencyFromProject(dynamic project)
        {
            ...
            catch (Exception ex)
            {
                var message = string.Format(
                    ...
                    Strings.Error_ProcessingNuspecFile,
                    project.FullPath,
                    ex.Message);
        ...
        private void ProcessDependencies(PackageBuilder builder)
        {
            ...
            var packagesAndDependencies = new Dictionary<String, Tuple<PackageArchiveReader, PackageDependency>>();
            ...
            foreach (var package in GetMinimumSet(packages))
            {
                ...
                if (dependencies.ContainsKey(package.GetIdentity().Id))
                {
                    continue;
                }
        ...
        {
            string filePath = Path.Combine(_project.FullPath, NuGetConstants.PackageReferenceFile);
            if (!File.Exists(filePath))
            {
                return;
            }
            Logger.LogMinimal(Strings.UsingPackagesConfigForDependencies);

            XDocument document = XDocument.Load(filePath);
            PackagesConfigReader configReader = new PackagesConfigReader(document);

            // Get the solution repository
            PackageArchiveReader repository = GetPackagesRepository();

            // Collect all packages
            IDictionary<PackageIdentity, PackageReference> packageReferences =
                configReader.GetPackages()
            ...
            foreach (PackageReference reference in packageReferences.Values)
            {
                if (repository != null)
                {
                    PackageArchiveReader package = null; // repository.GetReferenceItems(); // (reference.PackageIdentity.Id, reference.PackageIdentity.Version);
                    if (package != null && !packagesAndDependencies.ContainsKey(package.GetIdentity().Id))
                    {
                        VersionRange spec = GetVersionConstraint(packageReferences, package);
                        var dependency = new PackageDependency(package.GetIdentity().Id, spec);
                        packagesAndDependencies.Add(package.GetIdentity().Id, new Tuple<PackageArchiveReader, PackageDependency>(package, dependency));
                    }
                }
            }
        }

        private static VersionRange GetVersionConstraint(IDictionary<PackageIdentity, PackageReference> packageReferences, PackageArchiveReader package)
        {
            VersionRange defaultVersionConstraint = VersionRange.Parse(package.GetIdentity().Version.ToString());

            PackageReference packageReference;
            var key = new PackageIdentity(package.GetIdentity().Id, package.GetIdentity().Version);
            if (!packageReferences.TryGetValue(key, out packageReference))
            {
                return defaultVersionConstraint;
            }

            return packageReference.AllowedVersions ?? defaultVersionConstraint;
        }

        private IEnumerable<PackageArchiveReader> GetMinimumSet(List<PackageArchiveReader> packages)
        {
            return new List<PackageArchiveReader>();
                //Walker(packages, TargetFramework).GetMinimalSet();
        }
        ...
        private static void ProcessTransformFiles(PackageBuilder builder, IEnumerable<IPackageFile> transformFiles)
        {
            ...
            foreach (var transformGroup in transformGroups)
            {
                ...
                if (fileLookup.TryGetValue(transformGroup.Key, out file))
                {
                    ...
                    builder.Files.Add(new ReverseTransformFromFile(file, transformGroup));
                }
            }
        }
        ...
        private IEnumerable<IPackageFile> GetTransformFiles(PackageArchiveReader package)
        {
            List<IPackageFile> files = new List<IPackageFile>();
            IEnumerable<FrameworkSpecificGroup> groups = package.GetContentItems();
            foreach (var group in groups)
            {
                files.AddRange(group.Items.Where(IsTransformFile).Select(file => new PhysicalPackageFile
                {
                    SourcePath = file,
                    TargetPath = GetTargetPath(file)
                }));
            }
            return files;
        }
            ...
            if (!String.IsNullOrEmpty(solutionDir))
            {
                if (ProjectProperties.ContainsKey("SolutionDir"))
                {
                    Logger.LogWarning(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.Warning_DuplicatePropertyKey,
                            "SolutionDir"));
                }
        ...
        private PackageArchiveReader GetPackagesRepository()  // change this to just return a reader?
        {
            string solutionDir = GetSolutionDir();
            ...
            {
                return new PackageArchiveReader(target);
            }
        ...
        private Manifest ProcessNuspec(PackageBuilder builder, string basePath)
        {
            ...
            Logger.LogMinimal(
                string.Format(
                    CultureInfo.CurrentCulture,
                    Strings.UsingNuspecForMetadata,
        ...
        private void AddFiles(PackageBuilder builder, string itemType, string targetFolder)
        {
            ...
            var references = builder.PackageAssemblyReferences; //  packageReferenceFile.GetPackageReferences();
            PackageArchiveReader repository = GetPackagesRepository();
            string projectName = Path.GetFileNameWithoutExtension(_project.FullPath);

            IEnumerable<IPackageFile> contentFilesInDependencies = new List<IPackageFile>();
            if (references.Any() && repository != null)
            {
                var dependencies = repository.GetPackageDependencies();
                var packages = dependencies.Select(dependencyGroup => dependencyGroup.Packages.Select(package => package.Id));

                contentFilesInDependencies = references
                    .Select(referenceSet => referenceSet.References).ToList()
                    .Select(reference => new PhysicalPackageFile() { SourcePath = reference.FirstOrDefault() });
                /*
                    .Select(reference => repository.GetPackageDependencies()
                        .Select(dep => dep.Packages
                            .FirstOrDefault(pack => pack.Id == reference && pack.VersionRange == reference)))
                    .Where(a => a != null)
            ...
            foreach (var item in _project.GetItems(itemType))
            {
                ...
                if (!File.Exists(fullPath))
                {
                    Logger.LogMinimal(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.Warning_FileDoesNotExist,
                            targetFilePath));
                    continue;
                }
                ...
                if (targetFilePath.IndexOf("$(MSBuild", StringComparison.OrdinalIgnoreCase) > -1)
                {
                    Logger.LogWarning(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.Warning_UnresolvedFilePath,
                            targetFilePath));
                    continue;
                }
                ...
                IPackageFile targetFile = contentFilesInDependencies.FirstOrDefault(a => a.Path.Equals(targetPath, StringComparison.OrdinalIgnoreCase));
                if (targetFile != null)
                {
                    ...
                    if (isEqual)
                    {
                        Logger.LogMinimal(
                            string.Format(
                                CultureInfo.CurrentCulture,
                                Strings.PackageCommandFileFromDependencyIsNotChanged,
                                targetFilePath));
                        continue;
                    }
                    ...
                    Logger.LogMinimal(
                        string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.PackageCommandFileFromDependencyIsChanged,
                            targetFilePath));
                }
        ...
        private IPackageFile FindPackage(PackageArchiveReader repository, string hi)
        {
            var dependencies = repository.GetPackageDependencies();
            var packages = dependencies.Select(dependencyGroup => dependencyGroup.Packages.Select(package => package.Id == ""));
            return new PhysicalPackageFile() { SourcePath = "" };
        }
        ...
        private void AddFileToBuilder(PackageBuilder builder, PhysicalPackageFile packageFile)
        {
            ...
            {
                WriteDetail(Strings.AddFileToPackage, packageFile.SourcePath, packageFile.TargetPath);
        ...
        {
            if (_logger != null && LogLevel == LogLevel.Verbose)
            {
        ...
        private class ReverseTransformFromFile : IPackageFile
        {
            private readonly Lazy<Func<Stream>> _streamFactory;
            private readonly string _effectivePath;

            public ReverseTransformFromFile(IPackageFile file, IEnumerable<IPackageFile> transforms)
            {
