...
namespace NuGet.Commands
{
    public static class LockFileUtils
    {
            ...
            if (lockFileLib.CompileTimeAssemblies.Count > 0 || lockFileLib.RuntimeAssemblies.Count > 0)
            {
                ...
                if (groups.Count > 0)
                {
                    ...
                    if (referenceSet != null)
                    {
                        ...
                        lockFileLib.RuntimeAssemblies = lockFileLib.RuntimeAssemblies.Where(p => !p.Path.StartsWith("lib/", StringComparison.OrdinalIgnoreCase) || referenceFilter.Contains(Path.GetFileName(p.Path))).ToList();
                        lockFileLib.CompileTimeAssemblies = lockFileLib.CompileTimeAssemblies.Where(p => !p.Path.StartsWith("lib/", StringComparison.OrdinalIgnoreCase) || referenceFilter.Contains(Path.GetFileName(p.Path))).ToList();
                    }
                }
            }
        }
        ...
        {
            return group?.Any(item => !item.Path.EndsWith(PackagingCoreConstants.ForwardSlashEmptyFolder, StringComparison.OrdinalIgnoreCase)) == true;
        }
