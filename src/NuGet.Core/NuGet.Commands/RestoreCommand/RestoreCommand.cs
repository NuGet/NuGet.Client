...
#pragma warning restore CA1505
        {
            if (_request.Project.TargetFrameworks.Count == 0)
            {
                await _logger.LogAsync(RestoreLogMessage.CreateError(NuGetLogCode.NU1001, string.Format(CultureInfo.CurrentCulture, Strings.Log_ProjectDoesNotSpecifyTargetFrameworks, _request.Project.Name, _request.Project.FilePath)));
            ...
            context.ProjectLibraryProviders.Add(new PackageSpecReferenceDependencyProvider(GetProjectReferences(), _logger));
            ...
            // Loop through framework/runtime pairs where all RID-less frameworks are first, followed by the RID-specific ones
            foreach (var pair in CreateFrameworkRuntimePairs(_request.Project, RequestRuntimeUtility.GetRestoreRuntimes(_request)))
            {
                ...
                if (!string.IsNullOrEmpty(pair.RuntimeIdentifier))
                {
                    ...
                    string runtimeGraphPath = _request.Project.TargetFrameworks.FirstOrDefault(e => NuGetFramework.Comparer.Equals(e.FrameworkName, tfmNonRidGraph.Framework))?.RuntimeIdentifierGraphPath;
                ...
                Queue<ImportRefItem> refImportQueue = new Queue<ImportRefItem>(RefImportQueueSize);

                Dictionary<LibraryRangeIndex, FindLibraryCachedAsyncResult> allResolvedItems = new Dictionary<LibraryRangeIndex, FindLibraryCachedAsyncResult>(2048);
            ...
            ProcessDeepEviction:
                ...
                refImportQueue.Clear();
                chosenResolvedItems.Clear();

                refImportQueue.Enqueue(rootProjectRefItem);

                while (refImportQueue.Count > 0)
                {
                    maxOutstandingRefs = Math.Max(refImportQueue.Count, maxOutstandingRefs);

                    ImportRefItem importRefItem = refImportQueue.Dequeue();
                    LibraryDependency currentRef = importRefItem.Ref;
                    LibraryDependencyIndex currentRefDependencyIndex = importRefItem.DependencyIndex;
                    LibraryRangeIndex currentRefRangeIndex = importRefItem.RangeIndex;
                    LibraryRangeIndex[] pathToCurrentRef = importRefItem.PathToRef;
                    HashSet<LibraryDependencyIndex> currentSuppressions = importRefItem.Suppressions;
                    IReadOnlyDictionary<LibraryDependencyIndex, VersionRange> currentOverrides = importRefItem.CurrentOverrides;
                    bool directPackageReferenceFromRootProject = importRefItem.DirectPackageReferenceFromRootProject;
                    ...
                    if (chosenResolvedItems.TryGetValue(currentRefDependencyIndex, out var chosenResolvedItem))
                    {
                        ...
                        if ((chosenRefRangeIndex == currentRefRangeIndex) &&
                            EvictOnTypeConstraint(currentRef.LibraryRange.TypeConstraint, chosenRef.LibraryRange.TypeConstraint))
                        {
                        ...
                        if (evictOnTypeConstraint || !RemoteDependencyWalker.IsGreaterThanOrEqualTo(ovr, nvr))
                        {
                            ...
                            evictions[evictedLR] = (ImportRefItem.CreatePathToRef(pathToCurrentRef, libraryRangeOfCurrentRef), currentRefDependencyIndex, chosenRef.LibraryRange.TypeConstraint);
                            totalEvictions++;
                            if (deepEvictions > 0)
                            {
                                totalHardEvictions++;
                            ...
                            while (refImportQueue.Count > 0)
                            {
                                ImportRefItem item = refImportQueue.Dequeue();
                    ...
                    for (int i = 0; i < refItemResult.Item.Data.Dependencies.Count; i++)
                    {
                        ...
                        refImportQueue.Enqueue(new ImportRefItem()
                        {
                            Ref = dep,
                            DependencyIndex = depIndex,
                            RangeIndex = refItemResult.GetRangeIndexForDependency(i),
                            PathToRef = ImportRefItem.CreatePathToRef(pathToCurrentRef, libraryRangeOfCurrentRef),
                    ...
                    if (!string.IsNullOrEmpty(pair.RuntimeIdentifier))
                    {
                        ...
                        if (runtimeDependencies.Count > 0)
                        {
                            ...
                            GraphNode<RemoteResolveResult> rootNode = GraphNode<RemoteResolveResult>.Create(libraryRange, rootHasInnerNodes, hasParentNodes: false, refItemResult.Item);
                ...
                restoreTargetGraph.Flattened = newFlattened;

                restoreTargetGraph.Graphs = nGraph;
        ...
        internal class FindLibraryCachedAsyncResult
        {
