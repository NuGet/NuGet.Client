    ...
    internal static class ContentFileUtils
    {
            ...
            List<ContentItemGroup> contentFileGroups)
        {
            ...
            foreach (var filesEntry in nuspecContentFiles)
            {
                ...
                Debug.Assert(!string.IsNullOrEmpty(filesEntry.Include), "invalid contentFiles entry");
                ...
                foreach (var file in entryMappings.Keys)
                {
                    ...
                    Debug.Assert(file.StartsWith("contentFiles/", StringComparison.OrdinalIgnoreCase),
                        "invalid file path: " + file);
                    var relativePath = file.Length > 13 ? file.Substring(13, file.Length - 13) : file;
