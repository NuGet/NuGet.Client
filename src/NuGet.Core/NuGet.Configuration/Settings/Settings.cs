...
#pragma warning disable CS0618 // Type or member is obsolete
        ...
        public void UpdateSubsections(string section, string subsection, IReadOnlyList<SettingValue> values)
        {
            ...
            if (values.Any())
            {
                ...
                if (string.Equals(ConfigurationConstants.CredentialsSectionName, section, StringComparison.OrdinalIgnoreCase) &&
                    GetCredentialsItemValues(values, out var username, out var password, out var isPasswordClearText))
                {
        ...
        public void SetNestedSettingValues(string section, string subsection, IList<SettingValue> values)
        {
            ...
            if (!updatedCurrentElement)
            {
                ...
                {
                    isItemUnknown = false;
                    item = new CredentialsItem(subsection, username, password, isPasswordClearText);
                }
        ...
        private bool GetCredentialsItemValues(IEnumerable<SettingValue> values, out string username, out string password, out bool isPasswordClearText)
        {
