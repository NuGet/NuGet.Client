...
namespace NuGet.Packaging.Licenses
{
    public class LicenseExpressionEvaluator
    {
        ...
        /// <returns></returns>
        public static List<LicenseExpressionToken> ConvertInfixToPostfixLicenseExpression(LicenseExpressionToken[] infixTokens)
        {
            var postfixStack = new Stack<LicenseExpressionToken>();
            var infixStack = GetStackFromInfixTokens(infixTokens);

            var postfixExpression = new List<LicenseExpressionToken>();

            var valuesSinceLastOpeningBracket = 0;
            var operatorsSinceLastOpeningBracket = 0;
            while (infixStack.Count > 0)
            {
                ...
                switch (infixStack.Peek().TokenType)
                {
                    case LicenseTokenType.OPENING_BRACKET:
                        valuesSinceLastOpeningBracket = 0;
                        operatorsSinceLastOpeningBracket = 0;
                        postfixStack.Push(infixStack.Pop());
                        break;

                    case LicenseTokenType.CLOSING_BRACKET:
                        if ((valuesSinceLastOpeningBracket - 1) != operatorsSinceLastOpeningBracket)
                        {
                            throw new ArgumentException("Invalid expression. Brackets are incorrect.");
                        }

                        // pop until we hit the opening bracket
                        while (postfixStack.Count > 0 && postfixStack.Peek().TokenType != LicenseTokenType.OPENING_BRACKET)
                        {
                        ...
                        infixStack.Pop();
                        // TODO NK - This might be unnecesarry
                        valuesSinceLastOpeningBracket = 0;
                        operatorsSinceLastOpeningBracket = 0;
                    ...
                    case LicenseTokenType.VALUE:
                        postfixExpression.Add(infixStack.Pop());
                        valuesSinceLastOpeningBracket++;
                        break;

                    case LicenseTokenType.WITH:
                    case LicenseTokenType.AND:
                    case LicenseTokenType.OR:
                        if (postfixStack.Count == 0 || postfixStack.Peek().TokenType == LicenseTokenType.OPENING_BRACKET)
                        {
                            postfixStack.Push(infixStack.Pop());
                        }  // An operator that has higher priority than the operator on the stack
                        else if (infixStack.Peek().TokenType < postfixStack.Peek().TokenType)
                        {
                            postfixStack.Push(infixStack.Pop());
                        ...
                        operatorsSinceLastOpeningBracket++;
                        break;

                    default:
                        throw new ArgumentException("Can never be reached");
                }
            }

            while (postfixStack.Count > 0)
            {
                if (postfixStack.Peek().TokenType != LicenseTokenType.OPENING_BRACKET)
                {
                    postfixExpression.Add(postfixStack.Pop());
                }
                else
