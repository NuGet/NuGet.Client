...
namespace NuGet.PackageManagement
{
    public sealed class NuGetFileLogger : IDisposable
    {
        ...
        private readonly string _fileName = "NuGet_dgSpec2_9200";
        ...
        public static List<int> AllValues { get; } = new List<int>();
        ...
        public void Write(string logMessage, int value = -1)
        {
            if (value > 0)
            {
                Values.Add(value);
        ...
        private StreamWriter CreateStreamWriter(string message)
        {
            if (IsEnabled)
            {
                ...
                var fileName = $"{_fileName}_{DateTime.UtcNow.Ticks:x}_{fullPath}.log";
