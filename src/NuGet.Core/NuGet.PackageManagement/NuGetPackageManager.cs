...
namespace NuGet.PackageManagement
{
    ...
    public class NuGetPackageManager
    {
            ...
            CancellationToken token)
        {
            ...
            {
                ReplayWarningsAndErrors(restoreResult.Result.LockFile, logger);
            }
        ...
        /// <summary>
        /// Function used to display errors and warnings at the end of Install operation.
        /// The errors and warnings are read from the assets file based on preview restore.
        /// </summary>
        ...
        private void ReplayWarningsAndErrors(LockFile lockFile, ILogger logger)
        {
            var logMessages = lockFile?.LogMessages ?? Enumerable.Empty<IAssetsLogMessage>();

            // Replay the log messages only if there were restore errors, because the assets file will not be committed.
            // If there were only warnings then those are written to assets file and committed. The design time build will replay them.
            if (logMessages.Any( m => m.Level == LogLevel.Error))
            {
                foreach (var logMessage in logMessages)
                {
                    ...
                    logger.LogAsync(restoreLogMessage);
                }
            }
        }
