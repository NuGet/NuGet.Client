    ...
    public sealed class PluginDiscoverer : IPluginDiscoverer
    {
        ...
        internal List<PluginFile> GetPluginsInNuGetPluginPaths()
        {
            ...
            foreach (var path in paths)
            {
                if (PathValidator.IsValidLocalPath(path) || PathValidator.IsValidUncPath(path))
                {
                    if (File.Exists(path))
                    {
                        ...
                        else
                        {
                            ...
                            pluginFiles.Add(new PluginFile(fileInfo.FullName, state, RequiresDotNetHost));
                        }
                    }
                ...
                {
                    pluginFiles.Add(new PluginFile(path, new Lazy<PluginFileState>(() => PluginFileState.InvalidFilePath), RequiresDotNetHost));
                }
            }
