...
namespace NuGet.DependencyResolver.Core.Tests
{
    public class FindPackageTests
    {
        ...
        [Fact]
        public async Task FindPackage_WhenExactVersionNotInCachedList_RefreshesHttpCacheAndResolves()
        {
            // Regression test for https://github.com/NuGet/Home/issues/3116.
            // The first call from a stale cache returns no match; after the resolver triggers
            // a refresh-on-miss the same source returns the now-published version.
            var range = new LibraryRange("x", VersionRange.Parse("1.0.2"), LibraryDependencyTarget.Package);
            var cacheContext = new SourceCacheContext();
            var testLogger = new TestLogger();
            var framework = NuGetFramework.Parse("net45");
            var context = new RemoteWalkContext(cacheContext, PackageSourceMapping.GetPackageSourceMapping(NullSettings.Instance), testLogger);
            var token = CancellationToken.None;
            var actualIdentity = new LibraryIdentity("x", NuGetVersion.Parse("1.0.2"), LibraryType.Package);
            var dependencies = new[] { new LibraryDependency() { LibraryRange = new LibraryRange("y", VersionRange.All, LibraryDependencyTarget.Package) } };
            var dependencyInfo = LibraryDependencyInfo.Create(actualIdentity, framework, dependencies);

            int findCallCount = 0;
            bool sawRefreshFlag = false;

            var remoteProvider = new Mock<IRemoteDependencyProvider>();
            remoteProvider.SetupGet(e => e.IsHttp).Returns(true);
            remoteProvider.SetupGet(e => e.Source).Returns(new PackageSource("test"));
            remoteProvider.Setup(e => e.FindLibraryAsync(range, It.IsAny<NuGetFramework>(), It.IsAny<SourceCacheContext>(), testLogger, token))
                .Returns<LibraryRange, NuGetFramework, SourceCacheContext, NuGet.Common.ILogger, CancellationToken>((_, _, ctx, _, _) =>
                {
                    findCallCount++;
                    if (ctx.RefreshMemoryCache)
                    {
                        sawRefreshFlag = true;
                        return Task.FromResult<LibraryIdentity>(actualIdentity);
                    }
            ...
            Assert.Equal(LibraryType.Package, result.Data.Match.Library.Type);
            Assert.Equal("1.0.2", result.Data.Match.Library.Version.ToString());
            Assert.True(sawRefreshFlag, "The cache context should have been refreshed before the second lookup.");
            Assert.Equal(2, findCallCount);
        }

        [Fact]
        public async Task FindPackage_WhenExactVersionStillNotFoundAfterRefresh_ReturnsUnresolved()
        {
            // Regression test for https://github.com/NuGet/Home/issues/3116:
            // when refresh-on-miss runs and the version is genuinely absent, we surface unresolved
            // (no extra refreshes, no NU1102 false-positives).
            var range = new LibraryRange("x", VersionRange.Parse("1.0.2"), LibraryDependencyTarget.Package);
            var cacheContext = new SourceCacheContext();
            var testLogger = new TestLogger();
            var framework = NuGetFramework.Parse("net45");
            ...
            remoteProvider.SetupGet(e => e.Source).Returns(new PackageSource("test"));
            remoteProvider.Setup(e => e.FindLibraryAsync(range, It.IsAny<NuGetFramework>(), It.IsAny<SourceCacheContext>(), testLogger, token))
                .Returns<LibraryRange, NuGetFramework, SourceCacheContext, NuGet.Common.ILogger, CancellationToken>((_, _, _, _, _) =>
                {
                    findCallCount++;
                    return Task.FromResult<LibraryIdentity>(null);
                });
        ...
        [Fact]
        public async Task FindPackage_WhenFloatingRangeIsSatisfiedFromStaleCache_DoesNotRefresh()
        {
            // Floating ranges should not trigger refresh-on-miss when the cached list satisfies them.
            // This guards against amplifying traffic on common restores.
            var range = new LibraryRange("x", VersionRange.Parse("1.0.0-*"), LibraryDependencyTarget.Package);
            var cacheContext = new SourceCacheContext();
            var testLogger = new TestLogger();
            var framework = NuGetFramework.Parse("net45");
            ...
            Assert.Equal(LibraryType.Package, result.Data.Match.Library.Type);
            // Floating range is already satisfied; we must not perform an extra refresh-on-miss query.
            Assert.Equal(1, findCallCount);
        }
