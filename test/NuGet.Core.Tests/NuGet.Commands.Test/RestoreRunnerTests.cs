...
";
        ...
        [Fact]
        public async Task RestoreRunner_Net5UsingNetCoreApp3WithFrameworkRefRaisesNU1701()
        {
            // Arrange
            var project1 = "project1";
            var packageSpec = @"
            ...
            using (var workingDir = TestDirectory.Create())
            {
                ...
                using (var cacheContext = new SourceCacheContext())
                {
                    ...
                    // NU1703 was raised.
                    Assert.Equal(NuGetLogCode.NU1703, lockFile.LogMessages[0].Code);
                    Assert.Equal(NuGetLogCode.NU1703, lockFile.LogMessages[1].Code);
                }
            }
        }
