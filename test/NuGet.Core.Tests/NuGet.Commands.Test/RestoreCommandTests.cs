...
namespace NuGet.Commands.Test
{
    ...
    public class RestoreCommandTests
    {
        ...
        [Fact]
        public async Task ExecuteAsync_WithPackageNamespaces_PopulatesTelemetry()
        {
            // Arrange
            using var context = new SourceCacheContext();
            using var pathContext = new SimpleTestPathContext();
...
<configuration>
    <config>
    <clear />
        <add key=""globalPackagesFolder"" value=""{pathContext.UserPackagesFolder}"" />
        <add key=""repositoryPath"" value=""{pathContext.PackagesV2}"" />
    </config>
    <packageSources>
        <clear />
        <add key=""source"" value=""{pathContext.PackageSource}"" />
    </packageSources >
    <packageNamespaces>
   ...
   <fallbackPackageFolders>
       <add key=""shared"" value=""{pathContext.FallbackFolder}"" />
   </fallbackPackageFolders>
</configuration>");

            var settings = new Settings(workingPath);
            ...
            var clientPolicyContext = ClientPolicyContext.GetClientPolicy(NullSettings.Instance, logger);
            var request = new TestRestoreRequest(packageSpec, new PackageSource[] { new PackageSource(pathContext.PackageSource, "source") }, pathContext.UserPackagesFolder, context, clientPolicyContext, packageNamespacesConfiguration, logger)
            {
                LockFilePath = Path.Combine(projectPath, "project.assets.json"),
                ProjectStyle = ProjectStyle.PackageReference
            };
