...
namespace NuGet.Protocol.Plugins.Tests
{
    public class MonitorNuGetProcessExitResponseTests
    {
        ...
        {
            var response = JsonSerializer.Deserialize(json, PluginJsonContext.Default.MonitorNuGetProcessExitResponse)!;
        ...
        public void JsonDeserialization_ThrowsForInvalidResponseCode(string json)
        {
            Assert.Throws<JsonException>(
                () => JsonSerializer.Deserialize(json, PluginJsonContext.Default.MonitorNuGetProcessExitResponse));
        }
    }
}
