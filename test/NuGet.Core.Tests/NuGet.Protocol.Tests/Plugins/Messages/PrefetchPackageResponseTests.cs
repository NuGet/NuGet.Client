...
{
    public class PrefetchPackageResponseTests
    {
        [Fact]
        public void Constructor_ThrowsForUndefinedResponseCode()
        {
            var exception = Assert.Throws<ArgumentException>(
                () => new PrefetchPackageResponse((MessageResponseCode)int.MinValue));
        ...
        {
            var response = new PrefetchPackageResponse(MessageResponseCode.Success);
        ...
        {
            var response = JsonSerializer.Deserialize(json, PluginJsonContext.Default.PrefetchPackageResponse)!;
        ...
        public void JsonDeserialization_ThrowsForInvalidResponseCode(string json)
        {
            Assert.Throws<JsonException>(
                () => JsonSerializer.Deserialize(json, PluginJsonContext.Default.PrefetchPackageResponse));
        }
    }
}
