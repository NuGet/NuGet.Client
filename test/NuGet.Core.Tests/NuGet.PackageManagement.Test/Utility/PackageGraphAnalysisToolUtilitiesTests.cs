...
{
    public class PackageGraphAnalysisToolUtilitiesTests
    {
        ...
        public void PackageGraphAnalysisUtilities_PopulateDependants_Succeeds()
        {
            ...
            foreach (var package in packageWithDependants)
            {
                switch (package.Identity.Id)
                {
                    case "a":
                        {
                            Assert.Equal(0, package.DependingPackages.Count);
                            break;
                        }
                    case "b":
                        {
                            Assert.Equal(1, package.DependingPackages.Count);
                            Assert.Equal(packageIdentityA100.Id, package.DependingPackages.Single().Id);
                            break;
                        }
                    case "c":
                        {
                    ...
                    case "d":
                        {
                            Assert.Equal(1, package.DependingPackages.Count);
                            Assert.Equal(packageIdentityB100.Id, package.DependingPackages.Single().Id);
                            break;
                        }
                    default:
                        {
