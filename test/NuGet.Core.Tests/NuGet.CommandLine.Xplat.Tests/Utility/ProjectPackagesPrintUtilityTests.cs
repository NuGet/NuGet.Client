...
namespace NuGet.CommandLine.Xplat.Tests.Utility
{
    public class ProjectPackagesPrintUtilityTests
    {
        ...
        public void BuildPackagesTable_Sponsor_OmitsVersionColumnsAndAlignsCells(bool printingTransitive, string expectedFirstHeader)
        {
            ...
            Assert.Contains(table, cell => cell.Value.Contains("Source: https://source", StringComparison.Ordinal));
            Assert.Contains(table, cell => cell.Value.Contains("https://sponsor/a", StringComparison.Ordinal));
        ...
        private static ListPackageArgs StandardListReportArgs => StandardListReportArgsCache ?? (StandardListReportArgsCache =
            new ListPackageArgs(
                        ...
                        prerelease: false, highestPatch: false, highestMinor: false, auditSources: null, logger: new Mock<ILogger>().Object,
                        packageSourceMapping: NoPackageSourceMapping, explicitPackageSources: Array.Empty<PackageSource>(), cancellationToken: CancellationToken.None));
