...
<configuration>
    <packageSources>
        <add key=""LocalSource"" value=""{pathContext.PackageSource}"" />
                ...
                Assert.Contains($"Installed x 1.0.0 ({resolver.GetNupkgMetadataPath(packageX.Id, NuGetVersion.Parse(packageX.Version))}) from {pathContext.PackageSource} with content hash {nupkg.ContentHash}.", result.AllOutput);
