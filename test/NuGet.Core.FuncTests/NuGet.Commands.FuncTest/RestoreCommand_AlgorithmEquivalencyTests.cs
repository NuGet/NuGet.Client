...
namespace NuGet.Commands.FuncTest
{
    public partial class RestoreCommandTests
    {
        ...
        // P -> C 1.0.0 (Project)
        //      -> B 2.0.0 (Project)
        //         -> D 1.0.0 PrivateAssets=All
        // B is pinned to 3.0.0
        [Fact]
        public async Task RestoreCommand_WithTransitivePinning_SuppressedPackagesAreRespected()
        {
            // Arrange
            using var pathContext = new SimpleTestPathContext();

            var B = ProjectTestHelpers.GetPackageSpecWithProjectNameAndSpec(
                "B",
                pathContext.SolutionRoot,
                @"
{
    ...
    ""net472"": {
      ...
      ""centralPackageVersions"": {
        ""B"": ""[2.0.0,)"",
            ...
            var C = ProjectTestHelpers.GetPackageSpecWithProjectNameAndSpec(
                "C",
                pathContext.SolutionRoot,
                @"
{
  ...
  ""frameworks"": {
    ""net472"": {
      ""centralPackageVersions"": {
            ...
            // Act & Assert
            (var result, _) = await ValidateRestoreAlgorithmEquivalency(pathContext, P, B, C);
            result.LockFile.Targets.Should().HaveCount(1);
            result.LockFile.Targets[0].Libraries.Should().HaveCount(3);
        }
