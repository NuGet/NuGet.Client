        ...
        public async Task RestoreCommand_RestoreFloatingVersionWithIgnoreFailingLocalSource()
        {
            ...
            using (var projectDir = TestDirectory.Create())
            {
                ...
                using (var context = new SourceCacheContext())
                {
                    ...
                    var provider = RestoreCommandProviders.Create(packagesDir, new List<string>(), sources.Select(p => cachingSourceProvider.CreateRepository(p)), context, new LocalPackageFileCache(), new RestoreCommandCache(), logger);
