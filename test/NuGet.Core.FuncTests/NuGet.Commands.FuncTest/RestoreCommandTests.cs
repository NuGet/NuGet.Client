...
";
        ...
        [Fact]
        public async Task Restore_WhenMappingNewSourceOverlappingPackageIds_DeletesPackageFromGPF()
        {
            // Arrange
            using var pathContext = new SimpleTestPathContext();
        ...
        public async Task RestoreCommand_RestoreWithPreviewSourceMapping_Succeeds()
        {
            //// Arrange
            //using var pathContext = new SimpleTestPathContext();
            //pathContext.Settings.AddPackageSourceMapping("source", "Contoso.*", "Test.*");

            //ISettings settings = Settings.LoadDefaultSettings(pathContext.SolutionRoot);            

            //var packageA = new SimpleTestPackageContext("Contoso.A", "1.0.0");
            //packageA.AddFile("lib/net472/a.dll");
            //packageA.AddFile("lib/netstandard2.0/b.dll");
            //var packageB = new SimpleTestPackageContext("Contoso.B", "1.0.0");
            ...
            //var spec = ProjectTestHelpers.GetPackageSpec("TestProject", pathContext.SolutionRoot, framework: "net5.0", dependencyName: packageA.Id, useAssetTargetFallback: true, assetTargetFallbackFrameworks: "net472");
            //var command = new RestoreCommand(ProjectTestHelpers.CreateRestoreRequest(spec, pathContext, packageSourceMappingConfiguration: PackageSourceMapping.GetPackageSourceMapping(settings), new TestLogger()));
            ...
            ////Unable to resolve 'a (>= 1.0.0)' for 'net5.0'. PackageSourceMapping is enabled, the following source(s) were not considered: C:\NuGet.Client\.test\work\acd8ef80\db3ca975\source.
            //result.Success.Should().BeTrue();
            ...
            using (var projectDir = TestDirectory.Create())
            {
                ...
                using (var context = new SourceCacheContext())
                {
                    ...
                    var provider = RestoreCommandProviders.Create(globalFolderPath: packagesDir, fallbackPackageFolderPaths: new List<string>(), sources.Select(p => cachingSourceProvider.CreateRepository(p)),
                        cacheContext: context, packageFileCache: new LocalPackageFileCache(), logger);
