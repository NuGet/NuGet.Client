...
namespace Dotnet.Integration.Test
{
    ...
    public class DotnetAddPackageTests
    {
        ...
        public async Task AddPkg_V2LocalSourceFeed_WithRelativePath_NoVersionSpecified_Success()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                ...
                await SimpleTestPackageUtility.CreateFolderFeedV2Async(
                    customSourcePath, //not using solution source folder
        ...
        [Fact]
        public async Task AddPkg_V2LocalSourceFeed_WithRelativePath_NoVersionSpecified_Fail()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                var projectName = "projectA";
        ...
        [Fact]
        public async Task AddPkg_V2LocalSourceFeed_WithRelativePath_VersionSpecified_Success()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                var projectName = "projectA";
        ...
        [Fact]
        public async Task AddPkg_V2LocalSourceFeed_WithRelativePath_VersionSpecified_Fail()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                var projectName = "projectA";
                ...
                // Act
                CommandRunnerResult result = _fixture.RunDotnet(projectDirectory, $"add {projectFilePath} package {packageX} -s {sourceRelativePath} -v {packageX_V3.Version}", ignoreExitCode: true);
        ...
        [Fact]
        public async Task AddPkg_V2_V3_LocalSourceFeeds_WithRelativePath_NoVersionSpecified_Success()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                var projectName = "projectA";
                ...
                var customPackageX_SourcePath = Path.Combine(pathContext.WorkingDirectory, "CustompackagesX");
                var customPackageY_Sourcepath = Path.Combine(pathContext.WorkingDirectory, "CustompackagesY");
                var sourceRelativePathX = Path.Combine("..", "..", "CustompackagesX");
                var sourceRelativePathY = Path.Combine("..", "..", "CustompackagesY");

                // Generate V2 Package
                await SimpleTestPackageUtility.CreateFolderFeedV2Async(
                    customPackageX_SourcePath, //not using solution source folder
                    new PackageIdentity[] { packageX_V1, packageX_V2 });

                // Generate V3 Package
                await SimpleTestPackageUtility.CreateFolderFeedV3Async(
                    customPackageY_Sourcepath,
                    PackageSaveMode.Defaultv3,
                ...
                // Single source
                CommandRunnerResult result = _fixture.RunDotnet(projectDirectory, $"add {projectFilePath} package {packageX} -s {sourceRelativePathX}", ignoreExitCode: true);
                result.Success.Should().BeTrue(because: result.AllOutput);

                // Multiple sources
                result = _fixture.RunDotnet(projectDirectory, $"add {projectFilePath} package {packageY} -s {sourceRelativePathY};{sourceRelativePathX}", ignoreExitCode: true);
                ...
                sources.Should().Contain(e => e.Contains(customPackageX_SourcePath));
                sources.Should().Contain(e => e.Contains(customPackageY_Sourcepath));
        ...
        [Fact]
        public async Task AddPkg_V2LocalSourceFeed_WithDefaultSolutiuonSource_NoVersionSpecified_Success()
        {
            using (var pathContext = new SimpleTestPathContext())
            {
                var projectName = "projectA";
                ...
                // Act
                CommandRunnerResult result = _fixture.RunDotnet(projectDirectory, $"add {projectFilePath} package {packageX}", ignoreExitCode: true);
                result.Success.Should().BeTrue(because: result.AllOutput);
