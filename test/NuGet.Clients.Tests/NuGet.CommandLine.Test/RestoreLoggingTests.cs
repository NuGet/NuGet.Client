...
namespace NuGet.CommandLine.Test
{
    public class RestoreLoggingTests
    {
        ...
        [Fact]
        public async Task RestoreLogging_VerifyConflictErrorNU1107ForCentralTransitiveAsync()
        {
            // Arrange
            using (var pathContext = new SimpleTestPathContext())
            {
                // Set up solution, project, and packages
                var solution = new SimpleTestSolutionContext(pathContext.SolutionRoot);
                string targetAlias = "net8.0";
                var net8 = NuGetFramework.Parse(targetAlias);
                ...
                // Enable Central Package Version Management with transitive pinning
                projectA.Properties.Add("ManagePackageVersionsCentrally", "true");
                projectA.Properties.Add("CentralPackageTransitivePinningEnabled", "true");

                // Create Directory.Packages.props with package versions
                var centralPackageManagementFile = CentralPackageVersionsManagementFile.Create(pathContext.SolutionRoot, true);
                centralPackageManagementFile.SetPackageVersion("x", "1.0.0");
                centralPackageManagementFile.SetPackageVersion("y", "1.0.0");
                centralPackageManagementFile.SetPackageVersion("z", "1.0.0"); // This version will conflict
                centralPackageManagementFile.Save();
                ...
                // Add package references without version (versions are centrally managed)
                projectA.AddPackageToAllFrameworks(new SimpleTestPackageContext("x", null));
                projectA.AddPackageToAllFrameworks(new SimpleTestPackageContext("y", null));
                ...
                // Act
                var r = Util.RestoreSolution(pathContext, expectedExitCode: 1, testOutputHelper: _testOutputHelper);
                var log = projectA.AssetsFile.LogMessages.SingleOrDefault(e => e.Code == NuGetLogCode.NU1107);
                ...
                log.LibraryId.Should().Be("z");
                log.Level.Should().Be(LogLevel.Error);
                log.Message.Should().Contain("Version conflict detected for z transitively pinned centrally managed package");
                log.Message.Should().Contain("Update the centrally managed package version in Directory.Packages.props");
            }
        }
    }
}
