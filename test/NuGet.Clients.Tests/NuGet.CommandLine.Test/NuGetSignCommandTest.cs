...
namespace NuGet.CommandLine.Test
{
    public class NuGetSignCommandTest
    {
        ...
        [Fact]
        public void SignCommandArgParsing_TrustAnchors_SinglePath()
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchors = @"C:\certs\root-ca.cer";

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Single(signArgs.TrustAnchorPaths);
            Assert.Equal(@"C:\certs\root-ca.cer", signArgs.TrustAnchorPaths[0]);
        }

        [Fact]
        public void SignCommandArgParsing_TrustAnchors_MultipleSemicolonSeparated()
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchors = @"C:\certs\root1.cer;C:\certs\root2.cer;C:\certs\root3.cer";

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Equal(3, signArgs.TrustAnchorPaths.Count);
            Assert.Equal(@"C:\certs\root1.cer", signArgs.TrustAnchorPaths[0]);
            Assert.Equal(@"C:\certs\root2.cer", signArgs.TrustAnchorPaths[1]);
            Assert.Equal(@"C:\certs\root3.cer", signArgs.TrustAnchorPaths[2]);
        }

        [Theory]
        [InlineData(null)]
        [InlineData("")]
        public void SignCommandArgParsing_TrustAnchors_NullOrEmpty_ReturnsEmptyList(string trustAnchors)
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchors = trustAnchors;

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Empty(signArgs.TrustAnchorPaths);
        }

        [Fact]
        public void SignCommandArgParsing_TrustAnchorFingerprints_SingleFingerprint()
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchorFingerprints = Sha256Hash;

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Single(signArgs.TrustAnchorFingerprints);
            Assert.Equal(Sha256Hash, signArgs.TrustAnchorFingerprints[0]);
        }

        [Fact]
        public void SignCommandArgParsing_TrustAnchorFingerprints_MultipleSemicolonSeparated()
        {
            // Arrange
            var hash1 = Sha256Hash;
            var hash2 = "B591A6D40BF420404A011733CFB7B190D62C65BF0BCDA32B56C92B409B0F9DCB";
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchorFingerprints = $"{hash1};{hash2}";

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Equal(2, signArgs.TrustAnchorFingerprints.Count);
            Assert.Equal(hash1, signArgs.TrustAnchorFingerprints[0]);
            Assert.Equal(hash2, signArgs.TrustAnchorFingerprints[1]);
        }

        [Theory]
        [InlineData(null)]
        [InlineData("")]
        public void SignCommandArgParsing_TrustAnchorFingerprints_NullOrEmpty_ReturnsEmptyList(string fingerprints)
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchorFingerprints = fingerprints;

            // Act
            var signArgs = signCommand.GetSignArgs();

            // Assert
            Assert.Empty(signArgs.TrustAnchorFingerprints);
        }

        [Fact]
        public void SignCommandArgParsing_TrustAnchors_TrailingSemicolon_IgnoresEmpty()
        {
            // Arrange
            var signCommand = ArrangeSignCommand(Sha256Hash);
            signCommand.TrustAnchors = @"C:\certs\root.cer;";
            ...
            // Assert
            Assert.Single(signArgs.TrustAnchorPaths);
            Assert.Equal(@"C:\certs\root.cer", signArgs.TrustAnchorPaths[0]);
        }
    }
}
