...
{
    public class FixVulnerablitiesServiceTests
    {
            ...
            bool expected)
        {
            ...
            Assert.Equal(expected, FixVulnerablitiesService.IsToolAvailable(functions, ValidNuGetToolName, AcceptableGroups));
        }

        [Fact]
        public void IsToolAvailable_NullOrEmptyFunctions_ReturnsFalse()
        {
            Assert.False(FixVulnerablitiesService.IsToolAvailable(functions: null, ValidNuGetToolName, AcceptableGroups));
            Assert.False(FixVulnerablitiesService.IsToolAvailable(new List<CopilotFunctionDescriptor>(), ValidNuGetToolName, AcceptableGroups));
        }
        ...
        public void IsToolAvailable_NonMcpDescriptorWithMatchingName_ReturnsFalse()
        {
            ...
            Assert.False(FixVulnerablitiesService.IsToolAvailable(functions, ValidNuGetToolName, AcceptableGroups));
        }
        ...
        public void IsToolAvailable_NoiseBeforeMatch_ReturnsTrue()
        {
            ...
            Assert.True(FixVulnerablitiesService.IsToolAvailable(functions, ValidNuGetToolName, AcceptableGroups));
        }
    }
}
